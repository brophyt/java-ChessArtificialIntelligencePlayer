...
public class transTableMiniMax implements ChessAI {
	int maxDepth;
	int maxPlayerNumber;
	int maxDepthReached;
	...
	int maxDepthFinal;
	int bestUtility; 
	Hashtable<Long, Integer> transTableHigh = new Hashtable<Long, Integer>();
	Hashtable<Long, Integer> transTableLow = new Hashtable<Long, Integer>();
	
	public transTableMiniMax(int d, int i) {
		bestUtility = 0; 
		maxDepthFinal = d; 
		maxDepth = d; 
		maxDepthReached = 0;
	...
	private int maxValue(int depth, Position position) throws IllegalMoveException {
		...
		for (int i = 0; i < moves.length; i++) {
			try {
				position.doMove(moves[i]);
				if (transTableHigh.containsKey(position.getHashCode())) {
					highestUtility = transTableHigh.get(position.getHashCode());
				} else {
					numberStatesVisited++;
					highestUtility = Math.max(highestUtility, minValue(depth + 1, position));
					transTableHigh.put(position.getHashCode(), highestUtility);
				}
				position.undoMove();
			} catch (IllegalMoveException e) {
				System.out.println("illegal move");
	...
	private int minValue(int depth, Position position) throws IllegalMoveException {
		...
		for (int i = 0; i < moves.length; i++) {
			try {
				position.doMove(moves[i]);
				if (transTableLow.containsKey(position.getHashCode())) {
					lowestUtility = transTableLow.get(position.getHashCode());
				} else {
					numberStatesVisited++;
					lowestUtility = Math.min(lowestUtility, maxValue(depth + 1, position));
					transTableLow.put(position.getHashCode(), lowestUtility);
				}
				position.undoMove();
			} catch (IllegalMoveException e) {
				System.out.println("illegal move");
