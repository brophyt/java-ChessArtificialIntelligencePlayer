...
/********************************** Choose AI Type ***************************************************/
		...
		/**********************MiniMax**************************/
		moveMaker[Chess.BLACK] = new AIMoveMaker(new MiniMaxAI(3, 1));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new MiniMaxAI(3, 0));
		
		/**********************ABPrune**************************/
		moveMaker[Chess.WHITE] = new AIMoveMaker(new ABPruning(6, 0));
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new ABPruning(3, 1));

		
		
/********************************** Choose Initial Board Setup ****************************************/
		
		// full new game
		game = new ChessGame();
		
		// unique position for testing AB and MiniMax
		Position p = new Position("rnbqk1nr/2pppppp/pp6/7b/6P1/2PPP3/PP3PP1/RNBQKBNR w KQkq - 0 1");
		//game = new ChessGame(p);
		
		// test game with two white rooks and black king: 
		//Position p = new Position("2R5/k7/2K5/8/2R5/8/8/8 w - - 0 1");
		//game = new ChessGame(p);
		
		
		// test game with two black rooks and white king: 
//		 Position p = new Position("8/8/8/5r2/8/5k2/7K/4r3 w - - 0 1");
//		 game = new ChessGame(p);
		
		
		// test game just 2 kings:
		// Position p = new Position("4k3/8/8/8/4K3/8/8/8 w - - 0 1");
		// game = new ChessGame(p);
